...
const auth = (req, res, next) => { const value = req.headers.authorization || ''; const token = value.startsWith('Bearer ') ? value.slice(7) : ''; if (!token || !tokens.has(token)) return fail(res, 401, 'UNAUTHORIZED', 'Pharmacist authentication required'); next(); };
const stock = (medicineId, today = todayUtc()) => {
  const row = db.prepare(`SELECT COALESCE(SUM(CASE WHEN quantity > 0 AND expiry_date >= ? THEN quantity ELSE 0 END),0) sellable_stock, COALESCE(SUM(quantity),0) total_physical_stock FROM batches WHERE medicine_id=?`).get(today, medicineId);
...
app.post('/api/auth/login', (req,res) => { const body = req.body || {}; const user = process.env.PHARMACY_USER || 'pharmacist'; const pass = process.env.PHARMACY_PASSWORD || 'change-me'; if (body.username !== user || body.password !== pass) return fail(res,401,'INVALID_CREDENTIALS','Invalid pharmacist credentials'); const token = crypto.randomBytes(32).toString('hex'); tokens.set(token, Date.now() + 8*60*60*1000); return res.json({ token, expires_in: 28800 }); });
...
app.post('/api/medicines', auth, (req,res) => { const s=z.object({name:z.string().trim().min(1).max(200),generic_name:z.string().trim().max(200).optional().nullable(),unit:z.string().trim().min(1).max(40).default('unit')}).safeParse(req.body); if(!s.success)return fail(res,400,'VALIDATION_ERROR','Invalid medicine fields',s.error.flatten().fieldErrors); try { const r=db.prepare('INSERT INTO medicines(name,generic_name,unit) VALUES(?,?,?)').run(s.data.name,s.data.generic_name||null,s.data.unit); res.status(201).json(medicineView(db.prepare('SELECT * FROM medicines WHERE id=?').get(r.lastInsertRowid),todayUtc())); } catch(e) { if(String(e.message).includes('UNIQUE')) return fail(res,409,'DUPLICATE_MEDICINE','A medicine with this name already exists'); throw e; } });
...
const dispenseTx = db.transaction((medicineId, quantity, key, today) => {
  ...
  const r=db.prepare(`INSERT INTO dispense_transactions(medicine_id,quantity_requested,quantity_dispensed,status,idempotency_key) VALUES(?,?,?,?,?)`).run(medicineId,quantity,quantity,'success',key||null); const add=db.prepare('INSERT INTO dispense_lines(dispense_transaction_id,batch_id,quantity_taken) VALUES(?,?,?)'); for(const l of lines)add.run(r.lastInsertRowid,l.batch_id,l.quantity_taken); return { success:true,id:r.lastInsertRowid,lines };
});
app.post('/api/dispense', auth, dispenseLimiter, (req,res) => { const s=z.object({medicine_id:z.coerce.number().int().positive(),quantity:positiveInt,idempotency_key:z.string().trim().max(120).optional()}).safeParse(req.body); if(!s.success)return fail(res,400,'VALIDATION_ERROR','quantity must be a positive integer and medicine_id is required',s.error.flatten().fieldErrors); try { const out=dispenseTx(s.data.medicine_id,s.data.quantity,s.data.idempotency_key,todayUtc()); if(out.missing)return fail(res,404,'NOT_FOUND','Medicine not found'); if(out.prior)return res.json({dispensed:out.prior.quantity_dispensed,status:out.prior.status,transaction_id:out.prior.id,replayed:true}); if(out.rejected)return fail(res,409,'INSUFFICIENT_STOCK','Insufficient in-date stock',{available:out.available,transaction_id:out.id}); res.json({dispensed:s.data.quantity,status:'success',transaction_id:out.id,breakdown:out.lines}); } catch(e){ console.error(e); fail(res,500,'INTERNAL_ERROR','Unable to complete dispense'); } });
...
app.get('/api/alerts/expiring-soon',(req,res)=>{try{const days=alertDays(req.query.days),today=todayUtc(),end=new Date(`${today}T00:00:00Z`);end.setUTCDate(end.getUTCDate()+days);const data=db.prepare(`SELECT b.*,m.name medicine_name FROM batches b JOIN medicines m ON m.id=b.medicine_id WHERE b.quantity>0 AND b.expiry_date>=? AND b.expiry_date<=? ORDER BY b.expiry_date,b.received_date,b.id`).all(today,end.toISOString().slice(0,10));res.json({days,today,data});}catch(e){return fail(res,400,'VALIDATION_ERROR',e.message);}});
app.get('/api/alerts/expired',(req,res)=>res.json({today:todayUtc(),data:db.prepare(`SELECT b.*,m.name medicine_name FROM batches b JOIN medicines m ON m.id=b.medicine_id WHERE b.quantity>0 AND b.expiry_date<? ORDER BY b.expiry_date,b.id`).all(todayUtc())}));
app.get('/api/dispense-log',(req,res)=>{const page=Math.max(1,Number(req.query.page)||1),limit=Math.min(100,Math.max(1,Number(req.query.limit)||25)),total=db.prepare('SELECT COUNT(*) n FROM dispense_transactions').get().n;const data=db.prepare(`SELECT d.*,m.name medicine_name,COALESCE((SELECT json_group_array(json_object('batch_id',l.batch_id,'quantity_taken',l.quantity_taken)) FROM dispense_lines l WHERE l.dispense_transaction_id=d.id),'[]') lines FROM dispense_transactions d JOIN medicines m ON m.id=d.medicine_id ORDER BY d.id DESC LIMIT ? OFFSET ?`).all(limit,(page-1)*limit);res.json({data,pagination:{page,limit,total,pages:Math.ceil(total/limit)}});});
